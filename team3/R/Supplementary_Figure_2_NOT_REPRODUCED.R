...
# Load file
tcga <- read.table("team3/data/TCGA_clinical_final.out",
...
TCGA_pval <- data.frame(CANCER_TYPE_ACRONYM = rep(NA,
  times = length(unique(tcga$CANCER_TYPE_ACRONYM))
), pval = rep(NA, times = length(unique(tcga$CANCER_TYPE_ACRONYM))))
...
for (i in unique(tcga$CANCER_TYPE_ACRONYM)) {
  ...
  # makes a numeric vector of the above loop
  ...
  TCGA_pval$CANCER_TYPE_ACRONYM[counter] <- i
  TCGA_pval$pval[counter] <- pval
  ...
  plot_i <-
    ...
    theme_bw() +
    geom_vline(
      data = val, aes(xintercept = mean(val$value, na.rm = TRUE)),
  ...
  ggsave(
    filename = paste("results/Supplementary_Figure_2_", i, ".pdf", sep = ""),
    plot = plot_i,
    width = 5, height = 5
  )
}
write.csv(x = TCGA_pval, file = "team3/results/Supplementary_Figure_2_pval.csv")
